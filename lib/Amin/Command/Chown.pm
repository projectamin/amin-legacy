...
sub start_element {
	...
	$self->attrs(%attrs);
	if (($element->{Prefix} eq "amin") && ($element->{LocalName} eq "command") && ($attrs{'{}name'}->{Value} eq "chown")) {
...
sub end_element {
	...
	if (($element->{LocalName} eq "command") && ($self->command eq "chown")) {
		...
		if (($cmd->{TYPE} eq "out") || ($cmd->{TYPE} eq "both")) {
			my $otext = "Chown was successful for $ogroup on $target.";

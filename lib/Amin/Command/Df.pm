...
sub start_element {
	...
	$self->attrs(%attrs);
	if (($element->{Prefix} eq "amin") && ($element->{LocalName} eq "command") && ($attrs{'{}name'}->{Value} eq "df")) {
...
sub end_element {
	...
	if (($element->{LocalName} eq "command") && ($self->command eq "df")) {

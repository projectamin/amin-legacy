...
sub start_element {
	...
	$self->attrs(%attrs);
	if (($element->{Prefix} eq "amin") && ($element->{LocalName} eq "command") && ($attrs{'{}name'}->{Value} eq "head")) {
...
sub end_element {
	...
	if (($element->{LocalName} eq "command") && ($self->command eq "head")) {
		my $bytes = $self->{'BYTES'};
		my $lines = $self->{'LINES'};
		my $xflag = $self->{'FLAG'};
		...
		if (($cmd->{TYPE} eq "out") || ($cmd->{TYPE} eq "both")) {
			my $otext = "Head command was successful";

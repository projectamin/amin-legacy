...
sub start_element {
	...
	$self->attrs(%attrs);
	if (($element->{Prefix} eq "amin") && ($element->{LocalName} eq "command") && ($attrs{'{}name'}->{Value} eq "mount")) {
...
sub end_element {
	...
	if (($element->{LocalName} eq "command") && ($self->command eq "mount")) {
		...
		if (($cmd->{TYPE} eq "out") || ($cmd->{TYPE} eq "both")) {
			my $otext;
			if ($type) {
				$otext = "New partition is a $type partition. Mounting $device on $target.";
			} else {
				$otext = "Mounting $device on $target.";
			}

...
sub start_element {
	...
	$self->attrs(%attrs);
	if (($element->{Prefix} eq "amin") && ($element->{LocalName} eq "command") && ($attrs{'{}name'}->{Value} eq "rsync")) {
...
sub end_element {
	...
	if (($element->{LocalName} eq "command") && ($self->command eq "rsync")) {
		...
		if ($cmd->{TYPE} eq "error") {
			$self->{Spec}->{amin_error} = "red";
			my $text = "Rsync Failed. Reason: $cmd->{ERR}";
			$default = 1;
			$log->error_message($text);
			if ($cmd->{ERR}) {
...
Rsync - reader class filter for the rsync command.
...
=head1 DESCRIPTION
  ...
  A reader class for the rsync command. 

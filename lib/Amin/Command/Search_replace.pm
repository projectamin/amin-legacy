...
sub start_element {
	...
	$self->attrs(%attrs);
	if (($element->{Prefix} eq "amin") && ($element->{LocalName} eq "command") && ($attrs{'{}name'}->{Value} eq "search_replace")) {
...
sub characters {
	...
	if (($command eq "search_replace") && ($data ne "")) {
...
sub end_element {
	...
	if (($element->{LocalName} eq "command") && ($self->command eq "search_replace")) {

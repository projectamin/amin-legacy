...
sub start_element {
	...
	$self->attrs(%attrs);
	if (($element->{Prefix} eq "amin") && ($element->{LocalName} eq "command") && ($attrs{'{}name'}->{Value} eq "userdel")) {
...
sub end_element {
	...
	if (($element->{LocalName} eq "command") && ($self->command eq "userdel")) {
		my $dir = $self->{'DIR'};
		my $flag = $self->{'FLAG'};
		...
		if (($cmd->{TYPE} eq "out") || ($cmd->{TYPE} eq "both")) {
			my $otext = "Userdel command was successful";

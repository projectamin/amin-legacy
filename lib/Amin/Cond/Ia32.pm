...
sub start_element {
	...
	if (($element->{Prefix} eq "amin") && ($element->{LocalName} eq "cond") && ($attrs{'{}name'}->{Value} eq $arch)) {
		$state = 1;
...
sub characters {
	...
	if (($self->command eq "ia32") && ($data ne "")) {
		if ($state == 1) {
...
sub end_element {
	...
	if (($element->{LocalName} eq "cond") && ($self->command eq "ia32")) {
		$state = 0;
...
Ia32 - reader class filter for the x86 32bit  conditional.
...
=head1 DESCRIPTION
  ...
  A reader class for the x86 32 bit conditional. This is the 
  ...
  Amin::Cond::Ia32 for additional information. 

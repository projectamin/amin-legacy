...
sub start_element {
	...
	if (($element->{Prefix} eq "amin") && ($element->{LocalName} eq "cond") && ($attrs{'{}name'}->{Value} eq "os")) {
		$state = 1;
...
sub end_element {
	...
	if (($element->{LocalName} eq "cond") && ($self->command eq "os")) {
		$state = 0;
